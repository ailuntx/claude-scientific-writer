...
| `llm_model` | str | `None` | 模型名称（如 "anthropic/claude-sonnet-4.5"）用于图像描述 |
| `llm_prompt` | str | `None` | 图像描述的自定义提示词 |
...
**参数**:
- `source` (str): 要转换的文件的路径
- `file_extension` (str, optional): 覆盖文件扩展名检测
...
**参数**:
- `stream` (BinaryIO): 二进制类文件对象（例如以 `"rb"` 模式打开的文件）
- `file_extension` (str): 文件扩展名，用于确定转换方法（例如 ".pdf"）
...
3. **DocumentConverter 接口**: 改为从流读取而不是文件路径
   - 不再创建临时文件
   - 更节省内存
   - 插件需要更新
