---
name: markitdown
description: "将文件和办公文档转换为 Markdown。支持 PDF、DOCX、PPTX、XLSX、图像（带 OCR）、音频（带转录）、HTML、CSV、JSON、XML、ZIP、YouTube 链接、EPUB 等。"
...
MarkItDown 是微软开发的 Python 工具，用于将各种文件格式转换为 Markdown。它特别适合将文档转换为 LLM 友好的文本格式，因为 Markdown 令牌效率高且被现代语言模型广泛理解。

**主要优势**：
- 将文档转换为干净的、结构化的 Markdown
- 令牌效率高的格式，适合 LLM 处理
- 支持 15+ 种文件格式
- 可选的 AI 增强图像描述
- 图像和扫描文档的 OCR
...
通过 OpenRouter 使用 LLM 为 PPTX 和图像文件生成详细图像描述：
...
在 GitHub 上找到带有 hashtag 的插件：#markitdown-plugin
...
- **PDF 文件**：大型 PDF 可能需要时间；如支持可考虑使用页面范围
...
- 参阅 `references/api_reference.md` 获取完整 API 文档
- 查看 `references/file_formats.md` 获取格式特定详情
- 回顾 `scripts/batch_convert.py` 获取自动化示例
- 探索 `scripts/convert_with_ai.py` 获取 AI 增强转换
...
- **插件开发**：参阅 `packages/markitdown-sample-plugin`
