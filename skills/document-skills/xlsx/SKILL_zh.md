---
name: xlsx
description: "电子表格工具包（.xlsx/.csv）。使用公式/格式创建/编辑、分析数据、可视化、重算公式，用于电子表格处理和分析。"
...
#### 硬编码文档要求
- 注释或在单元格旁边（如果是表格末尾）。格式："来源：[系统/文档]，[日期]，[具体参考]，[URL（如果适用）]"
- 示例：
...
使用公式、格式和数据分析创建、编辑或分析 Excel 电子表格。将此技能应用于使用 openpyxl 和 pandas 进行电子表格处理。重新计算公式并确保发布质量的输出零错误。
...
- 只需用自然语言描述您想要的图表
- Nano Banana Pro 将自动生成、审查和改进原理图

**对于新文档：** 默认应生成科学原理图，以直观表示文本中描述的关键概念、工作流程、架构或关系。
...
**LibreOffice 是公式重新计算所必需的**：您可以假设 LibreOffice 已安装，用于使用 `recalc.py` 脚本重新计算公式值。该脚本首次运行时自动配置 LibreOffice
...
3. **修改**：添加/编辑数据、公式和格式
4. **保存**：写入文件
5. **重新计算公式（如果使用公式则为必选）**：使用 recalc.py 脚本
   ```bash
...
由 openpyxl 创建或修改的 Excel 文件包含公式字符串但不包含计算值。使用提供的 `recalc.py` 脚本重新计算公式：
...
- [ ] **列映射**：确认 Excel 列匹配（例如，列 64 = BL，而非 BK）
...
### 使用 openpyxl
- 单元格索引是基于 1 的（row=1, column=1 指单元格 A1）
