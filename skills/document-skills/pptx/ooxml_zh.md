...
**重要提示：开始前请阅读完整文档。** 整个文档涵盖了关键的 XML 架构规则和格式要求。错误的实现可能会创建 PowerPoint 无法打开的无效 PPTX 文件。
...
- **空白**：在带有前导/尾随空格的 `<a:t>` 元素中添加 `xml:space='preserve'`
- **Unicode**：转义 ASCII 内容中的字符：`"` 变成 `&#8220;`
...
6. **更新 `docProps/app.xml`**：增加幻灯片数量并更新统计信息（如存在）

### 复制幻灯片
1. 复制源幻灯片 XML 文件并重命名
2. 将新幻灯片中的所有 ID 更新为唯一值
3. 按照上述"添加新幻灯片"步骤操作
...
- **修复关系 ID**：
   - 如未使用嵌入字体，移除字体嵌入引用
- **移除无效引用**：检查所有 `_rels` 文件中是否有对已删除资源的引用
